...
class GUI(tk.Tk):
    ...
    def initiate_controls(self):
        ...
        btn4 = ttk.Button(self.control, text='Hint Solve', command=self.solve_hint)
        btn4.grid(row=3, column=1, padx=20, pady=20)

        btn5 = ttk.Button(self.control, text='upload hint', command=self.upload_hint)
    ...
    def solve_hint(self):
        ...
        solver = SolverV2()
        original_image = plt.imread(fileTest)
        result=solver.solve(original_image, x_cut, y_cut)
        cv2.imwrite(f'../results/{time.time()}.jpg', result)
        self.load_images()
